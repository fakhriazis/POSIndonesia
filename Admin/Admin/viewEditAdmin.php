...
<?= $this->section('content'); ?>
  ...
  <div class="container">
    <div class="card card-info">
      ...
      <!-- /.card-header -->
      <!-- form start -->
      <form class="form-horizontal" action="<?=base_url('/admin/csadmin/editadmin');?>" method="post">
        <div class="card-body">
          <?= csrf_field(); ?>
          <?php foreach ($listAdmin as $a): ?>
          <div class="form-group row">
            <input type="text" name="iduser" value="<?= $a->iduser ?>" hidden>
            <label for="InputName" class="col-sm-2 col-form-label">Nama</label>
            <div class="col-sm-10">
              <input name="nama_user" type="text" class="form-control" id="InputName" placeholder="Nama" value="<?= $a->nama_user?>" required>
            </div>
          </div>
          <div class="form-group row">
            <label for="inputEmail3" class="col-sm-2 col-form-label">Email</label>
            <div class="col-sm-10">
              <input name="email" type="email" class="form-control" id="inputEmail3" placeholder="Email" value="<?= $a->email?>" required>
            </div>
          </div>
          <div class="form-group row">
            <label for="inputPassword3" class="col-sm-2 col-form-label">Password</label>
            <div class="col-sm-10">
              <input name="password" type="password" class="form-control" id="inputPassword3" placeholder="Password" value="<?= $a->password ?>" required>
            </div>
          </div>
          <div class="form-group row">
